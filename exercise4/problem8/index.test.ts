...
describe("exercise4 - problem8", () => {
  ...
  test.each(cases)(".calc($first)", ({ first, calls }) => {
    ...
    expect(Object.getOwnPropertyNames(Object.getPrototypeOf(calc))).toEqual([
      "constructor",
      "add",
      "subtract",
      "multiply",
    ...
    expect(Object.getOwnPropertyNames(calc)).toEqual([]);

    calls.reduce((calc, call) => {
      let result: Calculator;

      if (call.method != "logResult") {
        result = calc[call.method](call.arg);
      } else {
        result = calc[call.method]();
